...
import { View, Text, Dimensions, ScrollView, Alert, TouchableOpacity, Image, RefreshControl } from 'react-native'
import { ActivityIndicator, Banner, Button, FAB, IconButton, Portal } from 'react-native-paper'
...
const MyDocumentId = () => {
  ...
  const selectedSoftCopy = softCopy.record?.map((copy) => ({ uri: url + copy.uri }))
  ...
  const getSoftCopy = async () => {
    await axios.get('/student/get-softcopy', {
  ...
  const handleSubmit = async () => {
    ...
    await axios.post('/student/submit-requirement', formData)
      .then(() => {
        setImages([])
        getSoftCopy()
      })
  ...
  const handleReSubmit = async (submit_id) => {
    ...
    await axios.post('/student/resubmit-requirement', formData)
      .then(() => {
    ...
    <View className="flex-1">
          ...
          <>
            {(softCopy.record && softCopy.submit_status !== 'confirm') && (
              <Banner
                ...
                <Text className="font-pregular text-sm">{softCopy.submit_status === 'review' && "Note: Please submit your document/s manually in the registrar's office." || softCopy.submit_status === 'resubmit' && "Note: Please Re-Upload your document/s and submit manually in the registrar's office."}</Text>
              </Banner>
            )}
            {softCopy.record && !reupload && (
              <View className="space-y-2">
                <View>
                  <Text className={`rounded-t-xl p-4 font-pmedium text-white capitalize ${softCopy.submit_status === "confirm" && 'bg-green-500' || softCopy.submit_status === "review" && 'bg-yellow-500' || softCopy.submit_status === "resubmit" && 'bg-red-500'}`}>
                    {softCopy.submit_status === 'review' && 'To Review' || softCopy.submit_status === 'confirm' && 'Confirmed' || softCopy.submit_status === 'resubmit' && 'Resubmit'}
                  </Text>
                  <View className="bg-white p-4 rounded-b-xl space-y-2">
                    <View className="flex-row items-center justify-between">
                      <Text className="font-pregular text-sm">Date Submitted:</Text>
                      <Text className="font-pregular text-sm">{formatDate(softCopy.created_at)}</Text>
                    </View>
                    {softCopy.submit_status === 'confirm' && (
                      <View className="flex-row items-center justify-between">
                        <Text className="font-pregular text-sm">Date Confirmed:</Text>
                        <Text className="font-pregular text-sm">{formatDate(softCopy.updated_at)}</Text>
                      </View>
                    )}
                  </View>
                </View>
                {softCopy.submit_status === 'resubmit' && (
                  <View className="bg-white space-y-4 p-4 rounded-xl">
                    ...
                    <Text className="text-sm font-pregular">{softCopy.submit_message}</Text>
                  </View>
                )}
                <View className="flex-row flex-wrap" style={{ gap: 8 }}>
                  {softCopy.record?.map((copy, index) => (
                    <TouchableOpacity activeOpacity={0.7} key={index} onPress={() => clickView(index)}>
                      <Image
            ...
            {images && (
              <View className={`flex-row flex-wrap ${images[0] ? 'pb-20' : 'pb-0'}`} style={{ gap: 8 }}>
                {images && images.map((image, index) => (
                  <View key={index} className="relative">
                    ...
                    />
                    <TouchableOpacity activeOpacity={0.7} onPress={() => clickView(index)}>
                      <Image
                        source={image}
                        contentFit="cover"
                        transition={1000}
            ...
            {((!softCopy.record && !images[0]) || reupload) && (
              <View className={`h-full justify-center items-center space-y-6 ${images[0] && 'hidden'}`}>
                <Text className="font-pregular text-sm">Please Upload your Document/s</Text>
      ...
      {images[0] && (
        <View className={(loading || btnLoading) && 'hidden'}>
          <Button onPress={() => { softCopy.submit_status === 'resubmit' ? handleReSubmit(softCopy.id) : handleSubmit() }} mode='contained' contentStyle={{ height: 56 }} className='absolute bottom-4 left-4 right-20 rounded-xl'>
            <Text className="text-sm font-pmedium">{softCopy.submit_status === 'resubmit' ? 'Re-Submit' : 'Submit'}</Text>
          </Button>
      ...
      <ImageView
        images={(!reupload ? softCopy.record && selectedSoftCopy || images[0] && images : images[0] && images)}

...
const Request = () => {
  ...
  const checkOutData = selectedCredentials.map((credential) => ({
    ...
    price: credential.amount,
    selectedPurposes: Object.keys(purposeChecked[credential.id] || {}).filter(
      (purposeId) => purposeChecked[credential.id][purposeId]
    ),
    copies: Object.keys(purposeChecked[credential.id] || {}).reduce((acc, purposeId) => {
              ...
              if (selectedPurposes.length > 0) {
                return (
                  ...
                  >
                    ...
                    <List.Item
                      left={props => <Text {...props} className="text-sm font-pregular">{credential.on_page === 1 ? 'Partial Amount:' : 'Total Amount:'}</Text>}

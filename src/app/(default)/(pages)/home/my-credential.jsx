...
const MyCredential = () => {
  ...
  const getCount = async () => {
    await axios.get('/student/get-request-count', {
      params: { status }
    })
      .then(({ data }) => {
  ...
  const getPayment = async () => {
    await axios.get('/student/get-payment-status')
      .then(({ data }) => {
  ...
  return (
    <>
      <View className="flex-1">
        <ScrollView refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />} contentContainerStyle={{ flexGrow: 1, backgroundColor: "#DBEAFE80", padding: 16, rowGap: 16 }}>
          {loading ? (
            ...
            <>
              {tabs.map((tab, index) => (
                <List.Item
                  ...
                  left={props => <Image {...props} source={icons[index]} resizeMode='contain' className="w-14 h-14" />}
                  right={props =>
                    <View className="flex-row items-center">
                      ...
                      <Ionicons {...props} name='chevron-forward-outline' size={20} />
                    </View>
                  }
                  borderless
